...
## 2024-05-18 - SSRF Auto-Redirect Bypass
**Vulnerability:** Even with URL validation, `HttpClient` automatically follows redirects by default. This allowed an attacker to provide a trusted URL (e.g., Google Photos) that responds with a 3xx redirect to a malicious or internal address, bypassing the domain allowlist and resulting in SSRF.
**Learning:** Validating the initial URL is insufficient if the HTTP client automatically follows redirects.
**Prevention:** Always set `AllowAutoRedirect = false` on `HttpClientHandler` when fetching resources from user-provided URLs.
